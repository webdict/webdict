...
import {Table, Divider, Tag, Icon, Form, Row, Col, Select, Dropdown, Menu, Input} from 'antd';
...
export default function Hist({
  ...
  function onTableChange({current}) {
    history.replace(`/history/_${current}`);
  }
  ...
  function maxTag(vals) {
    ...
    return `(${vals.length}) ${vals
      ...
      .reduce((a, b) => `${a}${a.slice(-1).charCodeAt(0) < 256 ? ', ' : '，'}${b}`)}`;
  }
  return (
    <React.Fragment>
      <Form style={{marginBottom: '8px'}}>
        <Row gutter={12}>
          ...
          <Col span={3}>
            <Input value={value} placeholder="搜索_%" onChange={onValueChange} />
          </Col>
          <Col span={11}>
            <Select
              mode="multiple"
