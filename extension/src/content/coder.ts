...
function _dPinv(pinv: string) {
  if (/^[a-z]{1,6}[0-5]$/.test(pinv)) {
    const pv = 'nue:nüe lue:lüe nv:nü lv:lü v:ü'.split(' ')
      .map(x => x.split(':')).reduce((pinv, [b, a]) =>
        pinv.replace(b, a), pinv);
...
export function dePinv(pinv: string, lang: string) {
  return pinv.split(';').map(x => x.trim()).filter(Boolean)
    .map(pinv => {
      const pinvs = pinv.split(',').map(x => x.trim()).filter(Boolean);
      let __pinv__ = null;
      return pinvs.map((pinv, i) => {
        const each = pinv => pinv.split(' ').map(x => x.trim()).filter(Boolean)
          .map(pinv => `<span class="lanx-pron-play" data-code="${lang + pinv}">${
            pinv.split('-').map(pinv => _dPinv(pinv)).join(' ')}</span>`).join(' ');
        if (i === 0) {
          const data = { '<': '繁体', '>': '简体', '=': '正体' };
          for (const mark of '<>=') {
          ...
          return `${each(pinv)}<span title="「${__pinv__ = __pinv__.split(' ').map(x => x.trim()).filter(Boolean).map(pinv => pinv.split('-').map(pinv => _dPinv(pinv)).join(' ')).join(' ')}」的异读音"> ※</span>`;
        } else {
...
function _deJyut(jyut: string, end: string) {
  if (/^[a-z]+[1-6]$/.test(jyut)) {
    ...
    return yut1.map((j, i) => j
      ? `<span class="lanx-jyut-${i + 1}">${j}${i === yut1.length - 1 ? end : ''}</span>`
      : '').join('');
  }
  return jyut + end;
}
...
export function deJyut(jyut: string, lang: string) {
  const yut1 = jyut.split(';').map(jyut => jyut.trim()).filter(jyut => jyut);
  return yut1.map((jyut, i1) => {
    const end1 = i1 + 1 !== yut1.length && ';' || '';
    const yut2 = jyut.split(',').map(jyut => jyut.trim()).filter(jyut => jyut);
    return yut2.map((jyut, i2) => {
      const end2 = i2 + 1 !== yut2.length && ',' || '';
      return jyut.split(' ').filter(jyut => jyut).map(jyut => {
        const yut3 = jyut.split('-').filter(jyut => jyut);
        return `<span class="lanx-pron-play" data-code="${lang + jyut}"${i2 !== 0
