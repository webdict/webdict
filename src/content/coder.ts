...
export function dePinv(pinv: string, lang: string) {
  return pinv.split(';').map(x => x.trim()).filter(x => x)
    .map(pinv => {
      const pinvs = pinv.split(',').map(x => x.trim()).filter(x => x);
      let __pinv__ = null;
      return pinvs.map((pinv, i) => {
        const each = pinv => pinv.split(' ').map(x => x.trim()).filter(x => x)
        ...
        if (i === 0) {
          ...
          return `${each(pinv)}<span title="「${__pinv__ = __pinv__.split(' ').map(x => x.trim()).filter(x => x).map(pinv => pinv.split('-').map(pinv => _dPinv(pinv)).join(' ')).join(' ')}」的异读音"> ※</span>`;
        } else {
