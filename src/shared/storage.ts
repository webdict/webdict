interface Item { [pron: string]: string | number; }
interface Section { [key: string]: Item; }
...
export function queryStorage(sectionKey: sectionKey, consume: (section?: Section, modify?: (section: Section) => void) => void) {
  try {
...
export function updateStorage(sectionKey: sectionKey, itemKey: string, update: (oldItem: Item, save: (newItem: Item) => void) => void) {
  try {
    chrome.storage.local.get(sectionKey, (storage: Storage) => {
